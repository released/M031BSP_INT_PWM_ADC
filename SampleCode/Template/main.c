...
#define FLAG_PROJ_PWM_DUTY_INC             				(flag_PROJ_CTL.bit2)
#define FLAG_PROJ_PWM_DUTY_DEC                          (flag_PROJ_CTL.bit3)
#define FLAG_PROJ_RPM_INC                               (flag_PROJ_CTL.bit4)
#define FLAG_PROJ_RPM_DEC                               (flag_PROJ_CTL.bit5)
#define FLAG_PROJ_REVERSE6                              (flag_PROJ_CTL.bit6)
#define FLAG_PROJ_REVERSE7                              (flag_PROJ_CTL.bit7)


/*_____ D E F I N I T I O N S ______________________________________________*/
volatile uint32_t g_u32SysTickMs = 0UL;
...
#define ADC_MAX_CODE                             (4095UL)
#define ADC_CALC_DATA_TO_VOLTAGE(DATA, VREF_MV)  \
    (((uint32_t)(DATA) * (uint32_t)(VREF_MV) + (ADC_MAX_CODE / 2UL)) / ADC_MAX_CODE)
...
void ADC_ReadAVdd(void)
{
    ...
    uint32_t u32BuiltInData;
    uint32_t u32Timeout = ADC_WAIT_READY_TIMEOUT_COUNT;
    ...
    while ((ADC_GET_INT_FLAG(ADC, ADC_ADF_INT) == 0UL) && (u32Timeout > 0UL))
    {
        u32Timeout--;
    }
    ...
    u32ConversionData = ADC_GET_CONVERSION_DATA(ADC, 29);
    ADC_CLR_INT_FLAG(ADC, ADC_ADF_INT);

    if (u32ConversionData == 0UL)
    {
        AVdd = 0UL;
        printf("%s : invalid conversion data\r\n", __FUNCTION__);
        return;
    }

    SYS_UnlockReg();
    FMC_Open();
    u32BuiltInData = FMC_ReadBandGap();	

	AVdd = (3072UL * u32BuiltInData + (u32ConversionData / 2UL)) / u32ConversionData;

	printf("%s : AVdd=%lumV, conv=%lu, bandgap=%lu\r\n",
           __FUNCTION__,
           (unsigned long)AVdd,
           (unsigned long)u32ConversionData,
           (unsigned long)u32BuiltInData);
...
void ADC_log(void)
{
    ...
    #if 1	// debug
    printf("AVdd:%4lumv-",(unsigned long)AVdd);
    for ( idx = 0 ; idx < ADC_CH_NUM ; idx++)
    {
        tmp = adc_measure[idx].data;

        printf("[%d]0x%03X,%4lumv,",
               idx,
               tmp,
               (unsigned long)ADC_CALC_DATA_TO_VOLTAGE(tmp, AVdd));
...
void Task_10ms_Callback(void *user_data)
{
    (void)user_data;
    /*
        TARGET_CH_0    - PB6 (ADC0_CH6)   
        TARGET_CH_1    - PB7 (ADC0_CH7) 
    */
...
void loop(void)
{
    ...
    RPM_Capture_Process(g_u32SysTickMs);
}
...
int main()
{
    ...
    */
    ...
    /*
        app_eint_capture.c
        PA5:PWM0_CH0 , for emulate singal OUTPUT       
        role#1 - PWM output with 100Hz , 120Hz@15% duty
   ...
   EINT_Init();
    ...
    /*
        app_rpm_capture.c
        PA7:TM2 , for emulate singal OUTPUT
        role#1 - PWM output with 1.8K@50% duty(as 100rpm),13.5K@50% duty(as 750rpm)
            - connect to PA8:TIM3_EXT

        PA8 : TIM3_EXT , for TIMER EXTERNAL CAPTURE INPUT , to calculate rpm 
        ex : 
            max : 72 * 750 * 15 / min = 750 rpm
            min : 72 * 100 * 15 / min = 100 rpm    
    */
    RPM_Capture_Init();
